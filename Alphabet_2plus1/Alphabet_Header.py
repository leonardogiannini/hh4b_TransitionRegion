                ...
                for i in range(plot.GetNbinsX()):  # Get pass/failed
				    ...
				    if which == ">":
                                        if plot.GetYaxis().GetBinCenter(j) < cut:
                                                ...
                                                print plot.Integral(i,i,j, j), plot.GetBinContent(i,j), i, j
...
def AlphabetSlicer3d(plot, bins, bins2,  cut, which, center, center2):
	print which, cut
	print "Alphabet2", plot, bins, bins2,  cut, which, center, center2
	print plot.Integral()
	...
	for b in bins: # loop through the bins (bins are along the mass axis and should contain a gap for the sigreg)
		b2=bins2[bins.index(b)]
		print b, b2
		print plot.GetNbinsX(), plot.GetNbinsY(), plot.GetNbinsZ()
		passed = 0
		failed = 0
		for i in range(plot.GetNbinsX()): 
#			print b[1], b[0],b2[1], b2[0]
			for j in range(plot.GetNbinsY()):   # Get pass/failed
				for k in range(plot.GetNbinsZ()):
					
#					print plot.GetXaxis().GetBinCenter(i), plot.GetYaxis().GetBinCenter(j), plot.GetZaxis().GetBinCenter(k)
					if plot.GetXaxis().GetBinCenter(i) < b[1] and plot.GetXaxis().GetBinCenter(i) > b[0]:
						if plot.GetYaxis().GetBinCenter(j) < b2[1] and plot.GetYaxis().GetBinCenter(j) > b2[0]:
							if which == ">":
								if plot.GetZaxis().GetBinCenter(k) < cut:
							...
							if which == "<":
								if plot.GetZaxis().GetBinCenter(k) > cut:
									failed = failed +  plot.GetBinContent(i,j, k)
#									print  plot.GetBinContent(i,j, k), i, j, k, plot.GetYaxis().GetBinCenter(j), plot.GetXaxis().GetBinCenter(i)
		...
		if failed < 0:
			print "FAIL"
			failed = 0
		if passed == 0 or failed == 0: continue
		print "QUALCOPSA"
		print passed, failed
		x.append((float((b[0]+b[1])/2.)-center))  # do the math in these steps (calculate error)
		exl.append(float((b[1]-b[0])/(12.)**0.5))
