...
binBoundaries =numpy.arange(437.5,2250,40.)
FILE = TFile("Hbb_output_mc.root", "RECREATE")
...
for b in range(len(Fit_bins)-1):
	...
	presel="cross_section<500000&&Inv_mass>"+str(Fit_bins[b])+"&&Inv_mass<"+str(Fit_bins[b+1])#+"&&(abs(cross_section-831.5)>0.5)"
	print presel
...
#	C1.cd()
	...
	C2.Print("fit_mc"+str(b)+"_bbtag%s.pdf"%cut[0])
	# Now we actually run the estiamte!
	# cuts:
	tag = "Inv_mass>"+str(Fit_bins[b])+"&&Inv_mass<"+str(Fit_bins[b+1])+"&&cross_section<500000&(fatjet_mass<130&fatjet_mass>90)&(dijet_mass<140&dijet_mass>100)&isCR<0.4"#+"&&(abs(cross_section-831.5)>0.5)"
	antitag = "Inv_mass>"+str(Fit_bins[b])+"&&Inv_mass<"+str(Fit_bins[b+1])+"&&cross_section<500000&(fatjet_mass<130&fatjet_mass>90)&(dijet_mass<140&dijet_mass>100)&isCR>0.4"#+"&&(abs(cross_section-831.5)>0.5)"
...
NDTT.Draw("hist same")
leg2.Draw()
C3.Print("bkg_mc_bbtag%s.pdf"%cut[0])
